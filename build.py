...
class App(tk.Tk):
    def __init__(self):
        ...
        try:
            import sys as _sys
            # PyInstaller place les fichiers embarqués dans sys._MEIPASS
            _base = getattr(_sys, "_MEIPASS", None) or os.path.dirname(os.path.abspath(__file__))
            _icon_paths = [
                os.path.join(_base, "icon.png"),
            ...
            for _p in _icon_paths:
                if os.path.exists(_p):
                    ...
                    _pil_img = _PILImage.open(_p).resize((32, 32), _PILImage.LANCZOS)
                    _ico = _PILImageTk.PhotoImage(_pil_img)
    ...
    def _build_ui(self):
        ...
        self.btn_update = tk.Button(header, text="🔄",
